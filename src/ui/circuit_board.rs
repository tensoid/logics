...
pub fn spawn_chip_at_cursor(
    ...
    mut commands: Commands, windows: Res<Windows>, 
    ...
    if input.just_pressed(MouseButton::Right) {
        ...
        if let Some(cursor_screen_pos) = window.cursor_position() {
            ...
            let cursor_world_pos: Vec2 = screen_to_world_space(window, camera, camera_transform, cursor_screen_pos);
...
pub fn screen_to_world_space(window: &Window, camera: &Camera, camera_transform: &GlobalTransform, position: Vec2) -> Vec2{
...
pub fn drag_chip(
    ...
    mut q_chips: Query<(&mut GlobalTransform, &ChipExtents), (With<Chip>, Without<Camera>)>
) {
    if input.just_pressed(MouseButton::Left) {
        ...
        if let Some(cursor_screen_pos) = window.cursor_position() {
            for (mut chip_transform, chip_extents) in q_chips.iter_mut() {
                let chip_position: Vec2 = Vec2::new(chip_transform.translation().x, chip_transform.translation().y);
                let cursor_position: Vec2 = screen_to_world_space(window, camera, camera_transform, cursor_screen_pos);

                let cursor_on_chip: bool = 
